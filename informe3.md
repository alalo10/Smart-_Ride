...
Finalmente, el alcance de este proyecto se enmarca en la validación tecnológica mediante una prueba de prototipo funcional (nivel de madurez TRL 4). Para ello, se implementará una maqueta funcional a escala que integrará la aplicación móvil, la lectura NFC (NTAG213), la base de datos y los servicios en la nube en un entorno controlado. Debido a la complejidad de esta integración simultánea y multidisciplinar, la delegación distribuida de los roles de backend, frontend, despliegue y pruebas entre los tres autores resulta indispensable. Este desarrollo de nivel TRL 4 sentará las bases institucionales para que la universidad pueda escalar el sistema hacia pruebas piloto operativas en el futuro, validando la captura de métricas precisas sobre el flujo diario de Vehículos de Movilidad Personal (VMP) y promoviendo el bienestar institucional y la movilidad sostenible.
...
4. **Fase 4: Pruebas, Validación y Documentación (Ciclo de Evaluación)**
   ...
   - Validación de Prototipo (TRL 4): Simulación de los escenarios de acceso, trazabilidad histórica y gestión de préstamos de VMP bajo condiciones técnicas controladas, utilizando una maqueta funcional a escala que valide la comunicación entre el hardware móvil NFC y la arquitectura en la nube.
